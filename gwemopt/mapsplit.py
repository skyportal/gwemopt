...
def similar_range(params, map_struct):
    ...
    region_nsides = int((regions / 12) ** 0.5)
    ...
    def group_neighbours(group):
        ...
        for i in group:
            neighbours = np.append(
                neighbours, hp.get_all_neighbours(region_nsides, int(i), nest=True)
            )
        return neighbours
    ...
    for group in groups:
        ...
        for i in range(len(sliced_array)):
            if i in group:
                ...
                combined_map = np.append(combined_map, np.zeros(len(sliced_array[0])))
