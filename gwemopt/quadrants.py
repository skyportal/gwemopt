...
class ZTFtile:
    def __init__(self, RA, Dec, config_struct, number=None):
        ...
        for i in range(len(self.quadrant_RA)):
            ...
            self._wcs[-1].wcs.crval = [
                self.quadrant_RA[i].value,
                max(min(self.quadrant_Dec[i].value, 90), -90),
            ]
            self._wcs[-1].wcs.cd = [
                [-self.quadrant_scale.to(u.deg).value, 0],
    ...
    def quadrant_centers(self):
        ...
        alpha = alphap + np.arctan2(
            -np.cos(theta) * np.sin(phi - phip),
            np.sin(theta) * np.cos(deltap)
            - np.cos(theta) * np.sin(deltap) * np.cos(phi - phip),
        )

        delta = (
            np.arcsin(
                np.sin(theta) * np.sin(deltap)
                + np.cos(theta) * np.cos(deltap) * np.cos(phi - phip)
            )
        ).to(u.deg)
    ...
    def _inside_nogaps(self, RA, Dec):
        ...
        for i in range(len(self._wcs)):
            footprint[(4 * i) : (4 * (i + 1)), :] = self._wcs[i].calc_footprint(
                axes=self.quadrant_size
            )
        ...
        corner_order = order(
            outer_corners[:, 0], outer_corners[:, 1], self.RA.value, self.Dec.value
        )[::-1]
...
class HP_coverage:
    ...
    def find_tiles(self, ztftiles, probability_target=0.9, verbose=False):
        ...
        while summed_probability < probability_target and len(tiles) < len(ztftiles):
            ...
            individual_prob = hpmapc[
                ztftiles[tile_order[0]].inside(self.RA, self.Dec)
            ].sum()
            if verbose:
                print(
                    "Adding tile %d (%f,%f): individual probability = %.3f, total probability = %.3f"
                    % (
                        tile_order[0],
            ...
            # probability values
            tile_values = self.get_tile_values(hpmapc, ztftiles)
    ...
    def percentile_area(self, level):
        ...
        """
        return (
            (self.hpmap >= self.percentile(level)).sum()
            * healpy.nside2pixarea(self.nside, degrees=True)
            * u.deg**2
        )
