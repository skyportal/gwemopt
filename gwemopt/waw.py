...
def detectability_maps(
    ...
    for i in range(len(t)):
        if verbose:
            print(
                " #### computing detmap {0:d} of {1:d} ({2:.1f} percent completed) ####".format(
                    i + 1, len(t), i / len(t) * 100.0
                ),
                end="\r",
            )
...
def construct_followup_strategy_tiles(
    skymap, detmaps, t_detmaps, tile_struct, T_int, T_available, min_detectability=0.01
):
    ...
    ridx = np.arange(
        ntiles
    )  # this keeps track of the original healpix indices corresponding to the region
    dm = np.empty([len(detmaps), ntiles])
    for i in range(len(detmaps)):
        dm[i] = gwemopt.tiles.compute_tiles_map(
            tile_struct, detmaps[i], func="np.mean(x)"
        )
    ...
    tot_obs_time = np.sum(np.diff(T_available)[::2]) * 86400.0
    ...
    slots_in_window = (np.diff(T_available)[::2] * 86400.0 / T).astype(int)
    n_slots = np.sum(slots_in_window)
    t0_slot = np.empty(n_slots)
    k = 0
    for i in range(n_windows):
        for j in range(slots_in_window[i]):
            t0_slot[k] = T_available[2 * i] + j * T / 86400.0
            k = k + 1
    ...
    for p in descending_prob_idx:
        try:  # find the time of best detectability
            ...
            i0_best = np.arange(len(available_slots))[available_slots][
                i_best
            ]  # original index of the best time slot
        except:
            continue
        ...
        if detp[i_best] < min_detectability:
            ...
            available_slots[
                i0_best
            ] = False  # mark the assigned slot as not available anymore
...
def construct_followup_strategy(
    skymap,
    detmaps,
    t_detmaps,
    Afov,
    ...
    nsides = 2.0 ** np.arange(2, 10)
    areas = hp.nside2pixarea(nsides, degrees=True)
    ratio = Afov / areas
    nside = np.min(nsides[ratio > 1.0])
    ...
    ridx = np.arange(hp.nside2npix(nside))[
        region
    ]  # this keeps track of the original healpix indices corresponding to the region
...
def sky_pos_cond_prob_gt(x, x0, ra, dec, nside=256, limit_to_region=None):
    ...
    for p in np.arange(npix)[region]:
        ...
        bandwidth = np.std(d) * npsamp ** (
            -1.0 / 5.0
        )  # Silverman's rule (FIXME: use a better estimate?)
        w = np.exp(-0.5 * (d / bandwidth) ** 2)

        if np.sum(w) > 0.0:  # do not normalize if the sum is 0
            w = w / np.sum(w)
