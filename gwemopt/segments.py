...
def get_skybrightness(config_struct, segmentlist, observer, fxdbdy, radec):
    ...
    for ii in range(len(tt) - 1):
        ...
        if (alt_target < 30.0) or (alt_moon < 30.0):
            total_mag, total_mag_error, flux_mag, flux_mag_error = (
                np.inf,
                np.inf,
                np.inf,
                np.inf,
            ...
            delta_mag = np.interp(
                moon.moon_phase * 100.0, moon_phases, moon_data_passband
            )
            delta_mag_error = 0.1 * delta_mag

            flux = (
                sun_data_passband[0]
                + sun_data_passband[1] * angle
                + sun_data_passband[2] * alt_target
                + sun_data_passband[3] * alt_moon
            )
            flux_zp = (
                sun_data_passband[0]
                + sun_data_passband[1] * 90.0
                + sun_data_passband[2] * 90.0
                + sun_data_passband[3] * 45.0
            )
            ...
            flux_error = np.sqrt(
                sun_data_passband_error[0] ** 2
                + sun_data_passband_error[1] ** 2 * angle**2
                + sun_data_passband_error[2] ** 2 * alt_target**2
                + sun_data_passband_error[3] ** 2 * alt_moon**2
            )
