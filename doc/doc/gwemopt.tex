...
From the plot on the left of Figure~\ref{fig:eff_ntiles}, it can be seen that, in a limited range up to about 35 tiles, considering more tiles results in better efficiency. However, beyond that, we no longer benefit from increasing the number of tiles. This effect is shown in the left plot, where the efficiency curves are shown for different numbers of tiles. The three lines showing the results of 16, 28 and 40 tiles are closer to each other than the bottom one where only four tiles are selected. This means that increasing the number of tiles from 4 to 16 improves the efficiency more than from 16 to 28. This is expected since, as the number of tiles increases, new tiles cover less and less probability and the limiting factors in the time allocation and scheduling algorithm become more important and the efficiency will not keep rising.
